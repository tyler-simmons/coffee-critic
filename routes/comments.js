var express = require("express");
//required for db queries to function
var router  = express.Router({mergeParams: true});
var Shop = require("../models/shop");
var Comment = require("../models/comment");
var middleware = require("../middleware");

//Comments New
router.get("/new", middleware.isLoggedIn, function(req, res){
    // find shop 
    console.log(req.params.id);
    Shop.findOne({"_id": req.params.id}, function(err, queriedShop){
        if(err){
            console.log(err);
        } else {
             res.render("comments/new.ejs", {queriedShop: queriedShop});
        }
    })
});



//Comments Create
router.post("/new",middleware.isLoggedIn,function(req, res){
   //lookup campground using ID
   Shop.findOne({"_id": req.params.id}, function(err, queriedShop){
       if(err){
           console.log(err);
           res.redirect("/shops");
       } else {
        Comment.create(req.body.comment, function(err, comment){
           if(err){
               req.flash("error", "Something went wrong");
               console.log(err);
           } else {
               //add username and id to comment
               comment.author.id = req.user._id;
               comment.author.username = req.user.username;
               //save comment
               comment.save();
               queriedShop.comments.push(comment);
               queriedShop.save();
               console.log(comment);
               req.flash("success", "Successfully added comment");
               res.redirect('/shops/' + queriedShop._id);
           }
        });
       }
   });
});



// COMMENT EDIT ROUTE
router.get("/:comment_id/edit", middleware.checkCommentOwnership, function(req, res){
   Comment.findOne({"_id": req.params.comment_id}, function(err, queriedComment){
      if(err){
          res.redirect("back");
      } else {
        res.render("comments/edit.ejs", {shop_id: req.params.id, comment: queriedComment});
      }
   });
});



// COMMENT UPDATE
router.put("/:comment_id", middleware.checkCommentOwnership, function(req, res){
   Comment.findOneAndUpdate({"_id": req.params.comment_id}, req.body.comment, function(err, updatedComment){
      if(err){
          res.redirect("back");
      } else {
          res.redirect("/shops/" + req.params.id );
      }
   });
});



// COMMENT DESTROY ROUTE
router.delete("/:comment_id", middleware.checkCommentOwnership, function(req, res){
    //findByIdAndRemove
    Comment.findOneAndRemove({"_id": req.params.comment_id}, function(err){
       if(err){
           res.redirect("back");
       } else {
           req.flash("success", "Comment deleted");
           res.redirect("/shops/" + req.params.id);
       }
    });
});

module.exports = router;